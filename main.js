...
const mobileOpenMeun = function (target) {
  ...
  nav.style.transform = `translateX(${
    document.querySelector(".btn-close") ? "0" : "100%"
  })`
}
...
document.querySelector(".nav").addEventListener("click", function (e) {
  ...
  if (item.classList.contains("has-sublist")) {
    //control sublist element by class sublist-hide/sublist-show
    if (item.classList.contains("has-sublist")) {
      //control sublist display CSS
      link.nextElementSibling.style.display = `${
        item.classList.contains("sublist-hide") ? "block" : "none"
      }`
      ...
      link.nextElementSibling.style.opacity = `${
        item.classList.contains("sublist-hide") ? "1" : "0"
      }`

      //control sublist max-height to do the transition
      link.nextElementSibling.style.maxHeight = `${
        item.classList.contains("sublist-hide") ? "500px" : "0"
      }`

      //control sublist arrow icon rotation
      link.querySelector(".nav__item-arrow").style.transform = `${
        item.classList.contains("sublist-hide")
          ? "rotate(180deg)"
          : "rotate(0deg)"
      }`
    }
